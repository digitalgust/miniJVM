...
public class Shader2 {
    ...
    public void init1() {
        ...
        int[] whd = {0, 0, 0};
        texture1[0] = GToolkit.gl_image_load(GToolkit.readFileFromJar("/res/h_0.png"), whd);
        // ===================
        // Texture 2
        // ===================

        texture2[0] = GToolkit.gl_image_load(GToolkit.readFileFromJar("/res/h_1.png"), whd);
    ...
    void display1() {
        ...
        glUniform1i(glGetUniformLocation(ourShader, GToolkit.toCstyleBytes("ourTexture1")), 0);
        GToolkit.checkGlError("gl_paint 1.3");
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texture2[0]);
        glUniform1i(glGetUniformLocation(ourShader, GToolkit.toCstyleBytes("ourTexture2")), 1);
        GToolkit.checkGlError("gl_paint 1.5");
        ...
        int modelLoc = glGetUniformLocation(ourShader, GToolkit.toCstyleBytes("model"));
        int viewLoc = glGetUniformLocation(ourShader, GToolkit.toCstyleBytes("view"));
        int projLoc = glGetUniformLocation(ourShader, GToolkit.toCstyleBytes("projection"));
...
//                nvgEndFrame(vg);
    ...
    void display14() {
        ...
        int fullTransformMatrixLocation = glGetUniformLocation(programID, GToolkit.toCstyleBytes("fullTransformMatrix"));
        glUniformMatrix4fv(fullTransformMatrixLocation, 1, GL_FALSE, projectionMatrix.mat, 0);
        GToolkit.checkGlError("disp 1.5");

        glBindVertexArray(vao[0]);
        GToolkit.checkGlError("disp 2");


        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferID[0]);
        GToolkit.checkGlError("disp 5");

        glDrawElements(GL_TRIANGLES, shape.indices.length, GL_UNSIGNED_SHORT, null, 0);
        GToolkit.checkGlError("disp 6");
        ...
        GLMath.mat4x4_perspective(projectionMatrix.mat, 30.0f, ((float) fbWidth) / fbHeight, 0.1f, 10.0f);
        GLMath.mat4x4_mul(projectionMatrix.mat, projectionMatrix.mat, modelTransformMatrix.mat);
        fullTransformMatrixLocation = glGetUniformLocation(programID, GToolkit.toCstyleBytes("fullTransformMatrix"));
        glUniformMatrix4fv(fullTransformMatrixLocation, 1, GL_FALSE, projectionMatrix.mat, 0);
        glDrawElements(GL_TRIANGLES, shape.indices.length, GL_UNSIGNED_SHORT, null, 0);
