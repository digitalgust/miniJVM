/*this file is hand-maintained (callbacks, wrapper logic and the method
  table below); it is NOT regenerated by GenJniCode. Access the miniJVM object
  layout through jarray_body()/jarray_length() only.*/
#include <stdio.h>
#include <string.h>
...
int org_mini_media_MiniAudio_ma_decoder_init_file(Runtime *runtime, JClass *clazz) {
    ...
    if (ma_decoder_init_file(jarray_body(path), NULL, handle_decoder) != MA_SUCCESS) {
        env->push_long(runtime->stack, 0);
    } else {
        env->push_long(runtime->stack, (s64) (intptr_t)
                handle_decoder);
...
int org_mini_media_MiniAudio_ma_decoder_init_file_ex(Runtime *runtime, JClass *clazz) {
    ...
    ma_decoder *handle_decoder = env->jvm_calloc(sizeof(ma_decoder));
    if (ma_decoder_init_file(jarray_body(path), &mdc, handle_decoder) != MA_SUCCESS) {
        env->push_long(runtime->stack, 0);
    } else {
        env->push_long(runtime->stack, (s64) (intptr_t)
                handle_decoder);
...
int org_mini_media_MiniAudio_ma_decoder_init_memory(Runtime *runtime, JClass *clazz) {
    ...
    ma_decoder *handle_decoder = env->jvm_calloc(sizeof(ma_decoder));
    if (ma_decoder_init_memory(jarray_body(data), jarray_length(data), NULL, handle_decoder) != MA_SUCCESS) {
        env->push_long(runtime->stack, 0);
    } else {
...
int org_mini_media_MiniAudio_ma_decoder_init_memory_ex(Runtime *runtime, JClass *clazz) {
    ...
    ma_decoder *handle_decoder = env->jvm_calloc(sizeof(ma_decoder));
    if (ma_decoder_init_memory(jarray_body(data), jarray_length(data), &mdc, handle_decoder) != MA_SUCCESS) {
        env->push_long(runtime->stack, 0);
    } else {
        env->push_long(runtime->stack, (s64) (intptr_t) handle_decoder);
    }
...
int org_mini_media_MiniAudio_ma_decoder_get_para(Runtime *runtime, JClass *clazz) {
    ...
    if (handle_decoder && jarray_length(arr) >= 3) {
...
int org_mini_media_MiniAudio_ma_engine_play_sound(Runtime *runtime, JClass *clazz) {
    ...
    ma_result r = ma_engine_play_sound(handle_engine, jarray_body(path), NULL);
...
int org_mini_media_MiniAudio_ma_sound_init_from_file(Runtime *runtime, JClass *clazz) {
    ...
    if (ma_sound_init_from_file(handle_engine, jarray_body(path), pflag, handle_group, pDoneFence, handle_sound) != MA_SUCCESS) {
...
int org_mini_media_MiniAudio_ma_sound_get_position(Runtime *runtime, JClass *clazz) {
    ...
    if (farr && farr->mb.arr_type_index == DATATYPE_FLOAT && jarray_length(farr) >= 3) {
