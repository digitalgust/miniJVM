...
int sslc_init(SSLC_Entry *entry) {
    ...
    if (!ca_inited) {
        ...
        if (ca_len + 1 != ca_crt_rsa_size) {
            SSL_LOG_PRINTF("ssl cert init error");
            fflush(stdout);
        }
...
//                                     mbedtls_test_cas_pem_len);
     ...
     * 1. Start the connection
     */
    SSL_LOG_PRINTF("  . Connecting to tcp/%s:%s... ", hostname, port);
    //fflush(stdout);
...
//            SSL_LOG_PRINTF("Last error was: %d - %s\n\n", ret, error_buf );
//        }
        return -1;
    }
...
int sslc_write(SSLC_Entry *entry, char *buf, int len) {
     ...
     */
    //SSL_LOG_PRINTF("  > Write to server:");
    //fflush(stdout);
    ...
    len = ret;
    //SSL_LOG_PRINTF(" %d bytes written\n\n%s", len, (char *) buf);
...
int sslc_read(SSLC_Entry *entry, char *buf, int len) {
     ...
     */
    //SSL_LOG_PRINTF("  < Read from server:");
    //fflush(stdout);
